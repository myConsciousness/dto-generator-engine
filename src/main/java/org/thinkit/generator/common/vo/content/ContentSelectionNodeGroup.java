...
public final class SelectionNodeGroup implements Iterable<SelectionNode>, IterableNode<SelectionNode>, Serializable {
    ...
    /**
     * 指定された {@code index} の数値に紐づく {@link SelectionNode} オブジェクトを返却します。
     * 存在しないインデックスを指定した場合は実行時に必ず {@link ArrayIndexOutOfBoundsException} が発生するため、
     * {@link #get(int) メソッドの実行前にが必ず有効なアクセス範囲を確認してください。
     *
     * @param index インデックス
     * @return 指定された {@code index} の数値に紐づく {@link SelectionNode} オブジェクト
     *
     * @exception ArrayIndexOutOfBoundsException 引数として指定された {@code index}
     *                                           に紐づく情報が存在しない場合
     */
    public SelectionNode get(int index) {
        return this.selectionNodeGroup.get(index);
    }
